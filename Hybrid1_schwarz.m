% One level multiplicative Shcwarz method
...
% coarse space restriction operator 
crdC1 = crdC(unKnownsC);
crd1 = crd(unKnowns);
A = zeros(ndofC,ndof);
for i = 1:ndofC
    for j = 1:ndof
        ...
        if(i~=1 && i~=ndofC)
            ...
            if(x<=xi && x>=xim1)
                A(i,j) = (x-xim1)/(xi-xim1);
                continue;
            end
            if(x<=xip1 && x >=xi)
                A(i,j)=(xip1-x)/(xip1-xi);
                continue;
            end
        end
    end 
end
R0 = A(unKnownsC, unKnowns);

%computation of course stiffness matrix 
K0 = R0*K*R0';
